  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::uuv_sensor_ros_plugins_msgs::DVL_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.velocity_covariance.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.velocity_covariance[i]);
    }
    if (v.velocity_covariance.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.beams.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::uuv_sensor_ros_plugins_msgs::DVLBeam_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.beams[i]);
    }
    if (v.beams.empty() || false)
      s << "]";
  }
