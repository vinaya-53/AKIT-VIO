...
struct Printer< ::uuv_gazebo_ros_plugins_msgs::ThrusterConversionFcn_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::uuv_gazebo_ros_plugins_msgs::ThrusterConversionFcn_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "lookup_table_input: ";
    if (v.lookup_table_input.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.lookup_table_input.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.lookup_table_input[i]);
    }
    if (v.lookup_table_input.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "lookup_table_output: ";
    if (v.lookup_table_output.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.lookup_table_output.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.lookup_table_output[i]);
    }
    if (v.lookup_table_output.empty() || true)
      s << "]";
  }
