  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::uuv_gazebo_ros_plugins_msgs::UnderwaterObjectModel_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.added_mass.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.added_mass[i]);
    }
    if (v.added_mass.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "linear_damping: ";
    if (v.linear_damping.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.linear_damping.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.linear_damping[i]);
    }
    if (v.linear_damping.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "linear_damping_forward_speed: ";
    if (v.linear_damping_forward_speed.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.linear_damping_forward_speed.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.linear_damping_forward_speed[i]);
    }
    if (v.linear_damping_forward_speed.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "quadratic_damping: ";
    if (v.quadratic_damping.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.quadratic_damping.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.quadratic_damping[i]);
    }
    if (v.quadratic_damping.empty() || true)
      s << "]";
    if (true || !indent.empty())
