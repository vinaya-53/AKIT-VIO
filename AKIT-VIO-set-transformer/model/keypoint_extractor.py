...
class ORBKeypointExtractor:
    ...
    def __init__(self, max_keypoints=2000, descriptor_size=32):
        if cv2 is None:
        ...
        self.orb = cv2.ORB_create(
            ...
            scaleFactor=1.1,           # Smaller steps between scales for fine ripple textures
            nlevels=12,                # Increase levels to catch features at multiple distances
            edgeThreshold=10,          # Allow points closer to the edge
            firstLevel=0,
            WTA_K=2,
            scoreType=cv2.ORB_FAST_SCORE, # FAST_SCORE is more sensitive than HARRIS for sand ripples
            patchSize=31,
            fastThreshold=2            # CRITICAL: Lowest possible threshold to catch subtle gradients
        )
